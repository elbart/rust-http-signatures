...
// Just a wrapper enum for HTTPMethods
enum HTTPMethod {
    HEAD,
    GET,
    POST,
...
impl HTTPSignaturesImplementation {
    ...
    fn get_signature(&self, req: &HTTPRequest, headers: &Vec<HTTPHeader>)
        -> Result<String, String> {
        let string = self.get_string_to_sign(&req, headers);
        let mut hasher = Sha256::new();
        hasher.input_str(string.unwrap().as_str());
        Ok(b64_encode(&hasher.result_str()))
    }

    fn get_string_to_sign(&self, req: &HTTPRequest, headers: &Vec<HTTPHeader>)
        -> Result<String, String> {
    ...
    pub fn sign_request(&self, req: &HTTPRequest) -> Result<String, String> {
        ...
        let headers_string = headers_to_sign.iter()
            .map(|x| &x.0)
            .join(" ");

        let signature = self.get_signature(&req, &headers_to_sign);
        let result = format!(concat!("Signature keyId=\"{0}\",",
...
mod tests {
    ...
    fn test_process_headers_to_sign_get() {
        ...
        assert!(re.is_match(result[0].1.as_str()));
        assert_eq!(result[1].1.as_str(), "elbart.com");
    }
    ...
    fn test_process_headers_to_sign_post() {
        ...
        assert_eq!(result[0].1.as_str(), "2018-03-23");
        assert_eq!(result[1].1.as_str(), "elbart.com");
        assert_eq!(result[2].1.as_str(), &b64_encode("this is the body"));
    }
